...
const DinoGame: React.FC<DinoGameProps> = ({ isJumping, questionIndex, lives, onCollision }) => {
  ...
  const [checkpointDistance, setCheckpointDistance] = useState(220);
  ...
  useEffect(() => {
    if (gameContainerRef.current) {
      ...
      setCheckpointDistance(containerWidth / 5); // Divide by 5 to create five checkpoints
    }
  ...
  useEffect(() => {
    ...
    const checkpointPosition = (questionIndex % 5) * checkpointDistance;
    setCurrentPosition(checkpointPosition);
    setDinoPosition(checkpointPosition);
  }, [questionIndex, checkpointDistance]);
  ...
  useEffect(() => {
    const animate = (timestamp: number) => {
      ...
      if (!isJumping) {
        ...
        const nextCheckpoint = ((questionIndex % 5) + 1) * checkpointDistance;
        ...
        if (nextPosition < nextCheckpoint) {
          ...
          onCollision();
          const resetPosition = (questionIndex % 5) * checkpointDistance;
          setCurrentPosition(resetPosition);
          setDinoPosition(resetPosition);
        }
      }
  ...
  }, [isRunning, isJumping, questionIndex, currentPosition, checkpointDistance, onCollision]);

  return (
    <div ref={gameContainerRef} className="h-48 bg-gray-100 rounded-lg relative overflow-hidden">
      ...
      {[...Array(5)].map((_, i) => (
        <div
          ...
          style={{
            ...
            opacity: i <= (questionIndex % 5) ? 1 : 0.3
          }}
        />
      ))}
        ...
        <div className="h-4 bg-amber-800">
          <div className="absolute inset-0 flex items-center">
            {[...Array(35)].map((_, i) => (
              <div
                key={i}
                className="w-1 h-2 bg-gray-400 rounded-full mx-4"
              ></div>
            ))}
          </div>
        </div>
